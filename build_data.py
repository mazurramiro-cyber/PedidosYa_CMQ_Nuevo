...
STYLE=[("sin alcohol","0.0"),("sin gluten","rubia"),("0 0","0.0"),("cero","0.0"),("ipa","ipa"),("apa","apa"),("pale ale","apa"),("cream stout","stout"),("stout","stout"),("amber","amber"),("roja","roja"),("red","roja"),("negra","negra"),("noire","negra"),("golden","golden"),("oro","golden"),("dorada","golden"),("honey","honey"),("kolsch","kolsch"),("scotch","scotch"),("pure gold","puregold"),("session","session"),("pilsner","lager"),("pilsen","lager"),("rubia","rubia"),("lager","lager"),("chopp","rubia"),("clasica","rubia"),("original","rubia"),("light","light"),("ultra","light"),("belgian white","witbier"),("schwarzbier","negra"),("premium","lager"),("extra","lager")]
def norm(s):
    s=(s or "").lower()
...
def is_pack(t,dyn=""):
    nd=norm(t)
    if "pack" in nd or "sixpack" in nd or "combo" in nd: return True
...
def read_csv(path,fecha):
    ...
    with path.open("r",encoding="utf-8-sig",newline="") as f:
        for rec in csv.DictReader(f,delimiter=";"):
            desc=rec.get("descripcion") or ""; mcol=rec.get("marca") or ""
            texto=" ".join([x for x in [mcol,desc] if x and x.strip() not in ("-","")]).strip()
            if not texto: continue
            if is_pack(texto): continue
            marca=mcol.strip() if mcol.strip() not in ("-","") else infer(texto)
            if marca=="Guinnes": marca="Guinness"
            if marca=="Sin marca" and "cerveza" not in norm(texto): continue
            sku=clean(desc)
            if norm(sku) in BAD: continue
            ml=ml_text(rec.get("calibre")) or ml_text(desc)
            cal=bucket(ml)
            fleje=numf(rec.get("fleje")); precio=numf(rec.get("precio")); dnum=numf(rec.get("descuento"))
            d=(dnum/100.0) if dnum and dnum<100 else 0.0
            if precio is not None: ptc=precio
            elif fleje is not None: ptc=round(fleje*(1-d),2)
            else: ptc=None
    ...
    return rows
def build():
    csvs=sorted(SCRAPEOS_DIR.glob("pedidosya_cervezas_*.csv"))
    ...
    payload={"pivot":pivot,"dates":dates,"stats":stats,"fights":FIGHTS,"meta":{"generado":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),"sku_rows":len(pivot),"registros_validos":tot,"dias":len(dates),"plataforma":"PedidosYa"}}
    OUT_JSON.write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding="utf-8")
    return payload
if __name__=="__main__":
    d=build()
    print("[OK] data.json generado (PedidosYa)")
